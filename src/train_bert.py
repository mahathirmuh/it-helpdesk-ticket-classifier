...
def train_and_evaluate(texts, labels, model_name: str, epochs: int):
    ...
    model = BertClassifier(model_name=model_name, epochs=epochs)
    ...
    return model, report, report_dict, acc


def main(
    input_file: str = "data/cobacek.xlsx",
    output_file: str = "results/cobacek_bert_pred.xlsx",
    ...
    print("\n[BERT] Retrain pada full data & prediksi...")
    category_model.fit(df["description"].tolist(), df["category"].tolist())
    priority_model.fit(df["description"].tolist(), df["priority"].tolist())

    df["predicted_category"] = category_model.predict(df["description"].tolist())
    df["predicted_priority"] = priority_model.predict(df["description"].tolist())
    ...
    prediction_notes_map = {
        ...
        "predicted_category": "Hasil prediksi category oleh BERT",
        "predicted_priority": "Hasil prediksi priority oleh BERT",
    ...
    def write_output(file_path):
        with pd.ExcelWriter(file_path, engine="openpyxl") as writer:
            ...
            for col_idx, note in prediction_notes.items():
                if note:
                    pred_sheet.cell(row=1, column=col_idx).comment = Comment(note, "BERT")

            for sheet_name in ["Category Accuracy", "Priority Accuracy"]:
                sheet = writer.sheets[sheet_name]
                for col_idx, note in metric_notes.items():
                    sheet.cell(row=1, column=col_idx).comment = Comment(note, "BERT")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fine-tune BERT untuk klasifikasi tiket")
    parser.add_argument("--input",       default="data/cobacek.xlsx",                   help="Path file input Excel")
    parser.add_argument("--output",      default="cobacek_bert_pred.xlsx",              help="Path file output Excel")
    parser.add_argument("--bert-model",  default="distilbert-base-multilingual-cased",  help="HuggingFace model ID")
    parser.add_argument("--bert-epochs", type=int, default=3,                           help="Jumlah epoch fine-tuning")
